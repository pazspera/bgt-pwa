...
const faCircleCheckText = faCircleCheck.iconName;
...
describe("rendering",()=> {
  ...
  it("displays check icon if the player is registered", ()=> {
    const testWrapper = mountPlayerCard({ player: mockRegisteredSinglePlayer});
    
    expect(testWrapper.html()).toContain(faCircleCheckText);
    // checks circle check is not present on non registered players
    expect(wrapper.html()).not.toContain(faCircleCheckText);
  })
})
